...
std::vector<long long> post_table(0), subforum_table(0), user_table(0);
...
void NodeType() {
  ...
  for (i = 0; i < table_size; i++) {
    ...
    id = atoi(vocab[target].word);

    if (id >= padding * 2) { // user
      user_table.push_back(target);
    }
    else if (id >= padding) { // subforum
      subforum_table.push_back(target);
    }
    else if (id >= 0) { // post
      post_table.push_back(target);
    }
  ...
  std::cout << "post_table.size() " << post_table.size() << std::endl;
  std::cout << "subforum_table.size() " << subforum_table.size() << std::endl;
...
void InitUnigramTable() {
  ...
  for (a = 0; a < table_size; a++) {  // prob(table[a] = i) is proportional to the ratio of power(vocab[i].cn) / power_sum 
    table[a] = i;
...
// Learn words in the walks from the input file
void LearnVocabFromTrainFile() {
  ...
  while (1) {
    ReadWord(word, fin);  // read a word ending with 0 from fin
    if (feof(fin)) break;
    train_words++;
    ...
    if (i == -1) {
      ...
      vocab[a].cn = 1;
    } else vocab[i].cn++;  // if read before, count++; else add it to vocab and count = 1
...
void SaveVocab() {
  long long i;
  FILE *fo = fopen(save_vocab_file, "w");  // wb
...
void *TrainModelThread(void *id) {
  ...
  while (1) {
    if (word_count - last_word_count > 10000) {  // display training progress
      ...
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        if (negative > 0) for (d = 0; d < negative + 1; d++) { //  
          if (d == 0) {
            ...
            if (pp) {
              if (last_word >= padding * 2) { // user
                target = user_table[(next_random >> 16) % user_table.size()];
              }
              else if (last_word >= padding) { // subforum
                target = subforum_table[(next_random >> 16) % subforum_table.size()];
              }
              else if (last_word >= 0) {
                target = post_table[(next_random >> 16) % post_table.size()];
              }
...
void TrainModel() {
  ...
  fo = fopen(output_file, "w");  // wb
  char txt[5];
  strcpy(txt, ".txt\0");
  fp = fopen(strcat(output_file, txt), "w");  // wb
  if (classes == 0) {
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    printf("\t-padding <int>\n");
    printf("\t\tSet the gap of node number between different groups of nodes\n");
    printf("\t-debug <int>\n");
  ...
  if ((i = ArgPos((char *)"-padding", argc, argv)) > 0) padding = atoi(argv[i + 1]);
